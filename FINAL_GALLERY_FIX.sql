-- Final Gallery Fix Script - Drops old functions and recreates everything
...
-- Step 3: Copy data from alternative columns if they exist
DO $$
BEGIN
...
-- Step 4: Extract file_path from file_url if needed
UPDATE gallery_items
SET file_path =
  CASE
...
-- Step 5: Fix all file_urls to be complete URLs
UPDATE gallery_items
SET file_url =
  CASE
...
-- Step 6: Drop ALL existing versions of the functions
DROP FUNCTION IF EXISTS create_gallery_item CASCADE;
DROP FUNCTION IF EXISTS update_gallery_item CASCADE;
DROP FUNCTION IF EXISTS delete_gallery_item CASCADE;

-- Also drop with specific signatures if they exist
...
DROP FUNCTION IF EXISTS update_gallery_item(BIGINT, TEXT, TEXT, VARCHAR(50), TEXT[], BOOLEAN, BOOLEAN, INTEGER) CASCADE;
DROP FUNCTION IF EXISTS delete_gallery_item(BIGINT) CASCADE;

-- Step 7: Create fresh versions of the functions
...
-- Step 9: Final check - show what we have now
SELECT
  id,
  title,
  type,
  category,
  file_path,
  LEFT(file_url, 100) as file_url_preview,
  is_featured,
  is_published
FROM gallery_items
